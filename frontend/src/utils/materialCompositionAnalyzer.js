...
const MATERIAL_DATABASE = {
  ...
  'laptop': {
    ...
    specialInstructions: 'Remove battery before recycling. Contains valuable rare earth metals.',
    eWasteCategory: 'Large IT Equipment'
  },
  
  'cell phone': {
    materials: [
      { name: 'Lithium Polymer Battery', percentage: 20, hazard: 'high', recyclable: true },
      { name: 'Gorilla Glass', percentage: 15, hazard: 'low', recyclable: false },
      { name: 'Aluminum Frame', percentage: 25, hazard: 'low', recyclable: true },
      { name: 'Gold (Circuit Boards)', percentage: 2, hazard: 'low', recyclable: true },
      { name: 'Copper', percentage: 15, hazard: 'low', recyclable: true },
      { name: 'Plastic (Various)', percentage: 23, hazard: 'low', recyclable: true }
    ],
  ...
  'water bottle': {
    materials: [
      { name: 'PET Plastic (#1)', percentage: 95, hazard: 'low', recyclable: true },
      { name: 'HDPE Cap (#2)', percentage: 5, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'low',
    specialInstructions: 'Rinse before recycling. Remove cap if different plastic type.',
    plasticType: 'PET',
    recyclingCode: 1
  },
  ...
  'plastic bag': {
    materials: [
      { name: 'LDPE Plastic (#4)', percentage: 100, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'low',
    specialInstructions: 'Not accepted in curbside recycling. Return to grocery store drop-off.',
    plasticType: 'LDPE',
    recyclingCode: 4
  },
  ...
  'jeans': {
    materials: [
      ...
      { name: 'Polyester Blend', percentage: 25, hazard: 'low', recyclable: false },
      { name: 'Metal Rivets/Buttons', percentage: 5, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'medium',
    specialInstructions: 'Donate if wearable. Can be repurposed into insulation.',
  ...
  'wine bottle': {
    materials: [
      { name: 'Soda-lime Glass', percentage: 95, hazard: 'low', recyclable: true },
      { name: 'Cork/Synthetic Cork', percentage: 3, hazard: 'low', recyclable: false },
      { name: 'Aluminum Foil Cap', percentage: 2, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'low',
    specialInstructions: 'Remove cap and rinse. Glass is infinitely recyclable.',
    glassType: 'Soda-lime',
    glassColor: 'varies'
  },
  ...
  'battery': {
    materials: [
      ...
      { name: 'Plastic Casing', percentage: 10, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'high',
    specialInstructions: 'NEVER throw in trash! Fire hazard. Take to specialized e-waste facility.',
  ...
  'chair': {
    materials: [
      { name: 'Hardwood (Oak/Maple)', percentage: 60, hazard: 'low', recyclable: true },
      { name: 'Foam Padding', percentage: 20, hazard: 'low', recyclable: false },
      { name: 'Fabric Upholstery', percentage: 15, hazard: 'low', recyclable: true },
      { name: 'Metal Screws/Brackets', percentage: 5, hazard: 'low', recyclable: true }
    ],
    recyclingComplexity: 'medium',
    specialInstructions: 'Donate if usable. Disassemble for material-specific recycling.',
    furnitureType: 'Wood'
  }
};
...
const CATEGORY_TO_MATERIALS = {
  ...
  electronics: ['Lithium-ion Battery', 'Circuit Boards', 'Copper Wiring', 'Plastic Casing'],
  phone: ['Lithium Polymer Battery', 'Gorilla Glass', 'Aluminum', 'Precious Metals'],
  computer: ['Lithium-ion Battery', 'ABS Plastic', 'Aluminum', 'Copper', 'Rare Earths'],
...
export class MaterialCompositionAnalyzer {
  ...
  async analyze(image) {
    ...
    try {
      ...
      console.log('📦 Detected objects:', detections.map(d => d.class));

      // Step 3: Extract materials from knowledge base
      const materials = this.extractMaterials(topPrediction.className, detections);
      ...
      const recyclingRecommendations = this.generateRecyclingRecommendations(materials);
  ...
  extractMaterials(className, detections) {
    ...
    for (const [category, materials] of Object.entries(CATEGORY_TO_MATERIALS)) {
      if (normalizedName.includes(category)) {
        return {
          ...
          recyclingComplexity: 'medium',
          specialInstructions: 'Check local recycling guidelines for specific instructions.'
        };
      }
    }
    ...
    return {
      materials: [
        { name: 'Mixed Materials', percentage: 100, hazard: 'unknown', recyclable: true }
      ],
      recyclingComplexity: 'unknown',
      specialInstructions: 'Unable to determine specific materials. Please consult local recycling guidelines.'
    };
  }
  ...
  calculateEnvironmentalImpact(materials) {
    const recyclableMaterials = materials.materials?.filter(m => m.recyclable) || [];
    const recyclablePercentage = recyclableMaterials.reduce((sum, m) => sum + m.percentage, 0);
    
    const highHazardMaterials = materials.materials?.filter(m => m.hazard === 'high') || [];
    
    return {
      recyclablePercentage,
      co2SavedByRecycling: this.estimateCO2Savings(materials),
      landfillDiversionPotential: recyclablePercentage > 70 ? 'High' : recyclablePercentage > 40 ? 'Medium' : 'Low',
  ...
  estimateValueRecovery(materials) {
    const valuableMaterials = ['gold', 'silver', 'copper', 'aluminum', 'lithium', 'cobalt', 'rare earth'];
    ...
    return materials.recyclingComplexity === 'low' ? 'Medium - Standard recyclable materials' : 'Low';
  }
  ...
  identifyHazards(materials) {
    ...
    if (materials.dangerLevel === 'CRITICAL') {
      hazards.handlingInstructions.push('Store in cool, dry place away from flammable materials');
      hazards.handlingInstructions.push('If damaged, do not touch - call hazardous waste hotline');
    }
